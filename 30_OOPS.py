...
class computer:    # DEFINE A CLASS WITH ITS CLASS NAME
    def config(self):          #DEFINING a method name config that will show the confgiuration of the computer 
        print("Ram - 8gb\nRom - 1TB HDD & 120Gb SSD")      # WHAT THE FUNCTION WILL DO
computer_1 = computer()    # MENTIONING  that : "computer_1" is an obect of class "computer" 
computer_1.config() # Way-1 to call the function
computer.config(computer_1)     # Another way to call the function.   # Now  i am calling the function for a particular object from the class
                                    #  SYNTAX: class_name.method_name(object_name) 
...
# u can give arguements in while creating the object by giving the arguments directly in the parenthesis()
...
# #                 This method is defined in a class and can be used to initialize basuc variables
...
# Now there are two oobjects where it can be many more. and u want to call the function update() for a particular object.no wcomputer will choose random object.
...
# # #     (i) Instance variables --> these are instance variables because as the object changes the value change
class cars:
...
# # ii) CLASS METHODS --> Works with class variables. This methods are bound to class and not the objects in the class
# # A DECORATOR is required [@classmethod]
# # Instead of slef we will use "cls"
...
print(f"Somali is from {Somali.school_name()}")


# # iii) STATIC METHOD --> Nothing to do with class variablesor instancve variables
...
Ankan = students(55,48,98)
Shreya = students(100,99,98)
Rumana = students(99,100,98)

students.school = "ADAMAS"   

print(f"Ankan is from {Ankan.school_name()}")
print(f"Shreya is from {Shreya.school_name()}")
print(f"Rumana is from {Rumana.school_name()}")
...
class players:                     # OUTER CLASS
    ...
    def show(self):                # OUTER METHOD
        print(f"Player name- {self.name} and Goals scored- {self.goals}")   # Thsi is what the method will do
